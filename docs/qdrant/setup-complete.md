...
export class HybridEmbeddingManager {
  ...
  constructor(embeddingService: EmbeddingService, vectorOps: VectorOperations) {
...
export class PerformanceOptimizer {
  ...
  async monitorPerformance(collectionName: string) {
    ...
    if (stats.segmentsCount > 10) {
      console.log(
        `⚠️  High segment count (${stats.segmentsCount}). Consider optimization.`
      );
    }

    if (stats.pointsCount > 1000000) {
      console.log(
        `ℹ️  Large collection (${stats.pointsCount} points). Monitor memory usage.`
      );
    }
  }
}
