...
async function testEmbeddingGeneration(provider: EmbeddingProvider) {
  ...
  try {
    ...
    if (embedding.length === expectedDimensions) {
      ...
      console.error(`❌ Wrong embedding dimensions: expected ${expectedDimensions}, got ${embedding.length}`);
      return false;
    }
  } catch (error: any) {
...
async function testSearch(provider: EmbeddingProvider) {
  ...
  try {
    ...
    if (results.points.length > 0) {
      ...
      console.log(`   Best match: "${results.points[0].payload?.title}" (score: ${results.points[0].score?.toFixed(3)})`);
      return true;
    } else {
...
async function runTests() {
  ...
  if (hasWorkingSetup) {
    ...
    console.log('2. Use with Claude Code: claude "How do I implement slash commands?" --mcp-server ./mcp-server.ts');
  } else {
